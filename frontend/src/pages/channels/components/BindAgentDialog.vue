    ...
    <DialogContent class="sm:max-w-[480px] p-0 gap-0 overflow-hidden">
      <DialogHeader class="px-6 py-4">
        <DialogTitle class="text-lg font-semibold text-[#0a0a0a] dark:text-foreground">
          {{ t('channels.bindAgent.title', '选择助手') }}
        </DialogTitle>
        <DialogDescription class="hidden">{{ t('channels.bindAgent.desc', '请选择要绑定到该频道的助手') }}</DialogDescription>
      </DialogHeader>
      
      <div class="px-6 pb-6 pt-2 max-h-[400px] overflow-y-auto">
        <div v-if="loading" class="flex justify-center py-8">
          <span class="text-sm text-[#8c8c8c] dark:text-muted-foreground">{{ t('common.loading', '加载中...') }}</span>
        </div>
        <template v-else>
            ...
            <div class="flex flex-1 items-center gap-[11px] overflow-hidden">
              ...
              <div class="flex flex-col gap-[2px] overflow-hidden">
                <p class="text-sm leading-[22px] text-[#171717] dark:text-foreground">
                  {{ t('channels.bindAgent.autoGenerate', '自动生成助手') }}
                </p>
                <p class="text-xs leading-[20px] text-[#8c8c8c] dark:text-muted-foreground">
                  {{ t('channels.bindAgent.autoGenerateDesc', '根据频道名称自动创建并绑定一个 AI 助手') }}
                </p>
              </div>
            </div>
          ...
          <div v-if="agents.length === 0 && !fromCreate" class="flex justify-center py-8">
            <span class="text-sm text-[#8c8c8c] dark:text-muted-foreground">{{ t('channels.bindAgent.empty', '暂无助手，请先创建助手') }}</span>
          </div>
            ...
            <div class="flex flex-1 items-center gap-[11px] overflow-hidden">
              ...
              <div class="flex flex-col gap-[2px] overflow-hidden">
                ...
                <p class="truncate text-xs leading-[20px] text-[#8c8c8c] dark:text-muted-foreground">
                  {{ agent.prompt || t('channels.bindAgent.noDesc', '未设置描述') }}
                </p>
              </div>
            </div>
      ...
      <DialogFooter class="px-6 py-4 border-t border-[#f0f0f0] bg-white dark:border-border/50 dark:bg-muted/20">
        ...
        <Button 
          ...
          :disabled="!canConfirm"
        >
          {{ t('channels.bindAgent.confirm', '确认绑定') }}
        </Button>
      </DialogFooter>
    </DialogContent>
  </Dialog>
