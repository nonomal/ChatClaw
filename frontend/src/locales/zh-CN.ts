    ...
    general: {
      ...
      toolchain: {
        ...
        openclaw: {
          name: 'OpenClaw 运行环境',
          description:
            'OpenClaw Agent 的 Node.js 运行时环境，包含 openclaw CLI 和 Gateway。从 OSS 下载后安装到 ~/.chatclaw/openclaw/runtime/',
        },
      },
    },
    memory: {
  ...
  assistant: {
    ...
    settings: {
      ...
      advanced: {
        ...
        builtinTools: '内置工具',
        builtinToolsHint: '这里可直接选择 OpenClaw 内置工具；插件工具或自定义工具仍可在下方手动输入。',
  ...
  channels: {
    ...
    config: {
      ...
      dingtalkPluginInstallingDesc:
        '钉钉连接器插件正在后台安装，可稍后查看连接状态，现在可继续绑定助手',
    ...
    wechat: {
      ...
      step3:
        "连接成功后，微信里会自动出现新的「微信 ClawBot」对话，之后可再次通过扫码流程添加更多微信账号，或重新连接已有账号",
    ...
    whatsapp: {
      ...
      emptyDesc: '通过 OpenClaw 官方 @openclaw/whatsapp 插件扫码连接 WhatsApp（WhatsApp Web）。',
      addNow: '立即添加',
      configTitle: '配置 WhatsApp',
      configSubtitle: '使用 OpenClaw CLI：openclaw channels login --channel whatsapp',
      howToConnect: '如何连接',
      step1: '将安装官方插件：openclaw plugins install @openclaw/whatsapp',
      step2: '点击下方按钮后，应用会在后台运行扫码登录流程；请用手机 WhatsApp 扫描二维码。',
      ...
      pluginInstallTryLater: 'WhatsApp 插件正在安装或未就绪，请稍后再试。',
      channelNotFound: '未找到对应频道，请刷新后重试。',
      editNotSupported:
