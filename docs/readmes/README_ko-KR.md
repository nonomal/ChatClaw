...
### AI 채팅 어시스턴트
AI 어시스턴트에게 어떤 질문이든 하세요. 어시스턴트가 지식 베이스를 지능적으로 검색하여 관련 답변을 생성합니다.
![](../../images/previews/en/image1.png)

### 듀얼 모드 전환, 효율적인 작업 처리
채팅 모드는 멀티 시나리오 Q&A 및 추론에 적응; 태스크 모드는 내장 스킬 마켓과 짝을 이루어 AI 에이전트가 자율적으로 멀티 스텝 태스크를 분해하고 추진하여 효율을 향상시킵니다.
![](../../images/previews/en/image2.png)

### PPT 빠른 생성
스마트 어시스턴트에 한 문장 명령을 보내면 PowerPoint 프레젠테이션을 자동으로 만들고 생성합니다.
![](../../images/previews/en/image3.png)

### 스킬 매니저
명령으로 어시스턴트가 컴퓨터에 설치된 기능을 찾거나 새 확장 플러그인을 설치하도록 도와줍니다. 스킬 마켓 — 자유롭게 스킬을 탐색하고 설치.
![](../../images/previews/en/image4.png)

### 메모리 — 더 자연스럽고 스마트한 상호작용
맥락 대화를 실현하고 개인화 지원을 제공합니다. 지속적인 학습과进化 — 어시스턴트가 점점 더 사려 깊고 지능적인 서비스를 제공하는 성장하는 파트너처럼 느껴집니다.
![](../../images/previews/en/image5.png)

### 무료 모델 체험 — 공유 팀 지식 베이스
wán클릭으로 ChatWiki에 연결 승인, ChatWiki 계정 크레딧 동기화, 동시에 커스텀 모델도 지원. Ollama, Google Gemini, OpenAI 등을 포함한 고품질 국내외 LLMs 내장 — 일상 업무든 전문 시나리오든 좋아하는 AI 모델을 사용하세요.
![](../../images/previews/en/image6.png)

### 지식 베이스 | 문서 벡터화 저장
문서(TXT, PDF, Word, Excel, CSV, HTML, Markdown)를 업로드하면 시스템이 자동으로 파싱, 분할, 벡터화하여 프라이빗 지식 베이스에 저장. AI 모델의 정밀한 검색과 활용을 지원합니다.
![](../../images/previews/en/image7.png)

### 다양한 IM 채널 연동
인스턴트 메신저 서비스 제공업체(Feishu, WeCom, QQ, DingTalk, WeChat, WhatsApp 등)가 제공하는 SDK를 통합하여 채널 생성, 사용자 관리, 메시지 송수신을 포함한 완전한 IM 통신 기능을 앱 내에서 빠르게 구현합니다.
![](../../images/previews/en/image8.png)

### 예약 작업 — 자동화 명령 실행
어시스턴트가 사전 설정된 시간이나 간격으로 특정 작업을 자동으로 실행하도록 하세요. 적시 리마인더 제공, 주기적 작업 실행, 시스템 레벨 유지보수 작업 수행 등.
![](../../images/previews/en/image9.png)

### 텍스트 선택으로 즉시 Q&A
화면에서 임의의 텍스트를 선택하면 자동으로 플로팅 퀵 질문 상자에 복사됩니다. wan클릭으로 AI 어시스턴트에 보내면 즉시 답변을 받을 수 있습니다.
![](../../images/previews/en/image10.png)

### 스마트 사이드바
다른 앱 창에 스냅할 수 있는 스마트 어시스턴트. 서로 다른 설정의 AI 어시스턴트 간에 빠르게 전환하여 질문할 수 있습니다. 로봇이 연결된 지식 베이스에 따라 답변을 생성하고 wan클릭으로 대화창에 회신을 보내는 것도 지원합니다.
![](../../images/previews/en/image11.png)

### 한 질문에 여러 답변: 쉽게 비교
질문을 반복할 필요가 없습니다. 여러 'AI 전문가'에게 동시에 자문하고 동일한 인터페이스에서 답변을 나란히 볼 수 있습니다. 비교가 쉽고 최선의 결론에 도달하는 데 도움이 됩니다.
![](../../images/previews/en/image12.png)

### 원클릭 런처 볼
데스크톱의 플로팅 볼을 클릭하면 ChatClaw 기본 앱 창을 즉시 깨우거나 열 수 있습니다.
