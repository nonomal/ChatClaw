...
AI 어시스턴트에게 어떤 질문이든 하세요. 어시스턴트가 지식 베이스를 지능적으로 검색하여 관련 답변을 생성합니다. 내장 스킬 마켓과 함께 AI 에이전트가 자율적으로 작업하므로全程 개입이 필요하지 않습니다. 복잡한 프로젝트 기획, 문서 정리, PPT 생성, 다단계 작업 실행 등 모든 영역에서 AI 에이전트가 자율적으로 분석하고, 효율적으로 추진하며, 최종적으로 완전한 결과를 제공합니다.
...
독립적인 여러 AI 에이전트를 생성하여 각각 고유한 페르소나, 메모리, 스킬을 가집니다. 필요할 때 서로干扰 없이 전환하세요. 각 에이전트에 "고객 서비스", "데이터 분석가", "카피 어시스턴트" 등 다른 역할을 할당하고, 서로 다른 스킬, 지식 베이스, 응답 스타일을 구성할 수 있습니다.

![](../../images/previews/en/image3.png)

### Local Knowledge Base Management

문서(TXT, PDF, Word, Excel, CSV, HTML, Markdown)를 업로드하면 시스템이 자동으로 파싱, 분할, 벡터화하여 프라이빗 지식 베이스에 저장합니다. AI 모델의 정밀한 검색과 활용을 지원하며, 폴더 및 지식 베이스별 문서 정리를 지원합니다.

![](../../images/previews/en/image4.png)

### Skill Manager — AI Responds at Command

5000+ 즉시 사용 가능한 AI 스킬이 생산성, 개발 도구, 멀티미디어, 스마트 홈 등을 커버합니다. 프로그래밍 없이 AI에게 작업을 도와달라고 하세요. 명령어를 사용하여 설치된 기능을 찾거나 새 확장 플러그인을 설치하세요. 스킬 마켓에서 자유롭게 스킬을 검색하고 설치하세요.

![](../../images/previews/en/image5.png)

### Memory — Natural & Smarter Interactions

문맥 대화를 활성화하고 개인화된 서비스를 제공하며, 복잡한 작업을 완료하고 지속적으로 학습하고 발전합니다. 로봇은 점점 더 사려 깊고 지능적인 서비스를 제공하는 성장하는 동반자가 됩니다.

![](../../images/previews/en/image6.png)

### Free Model Trial

원클릭으로 ChatWiki에 연결하고 크레딧을 동기화하며 커스텀 모델을 지원합니다. Ollama, Google Gemini, OpenAI 등 고품질 내/외국산 LLM을 내장하고 있어 일상 업무든 전문 시나리오든 좋아하는 AI 모델을 사용하세요.

![](../../images/previews/en/image7.png)

### Multi-Channel Remote Control via WeChat/QQ/WhatsApp

ChatClaw은 다양한 메시지 채널을 지원하여 분석 결과, 모니터링 알림, 연구 요약을 직접 전화로 전송합니다. AI 처리 후 결과는 자동으로 지정 채널로 전송됩니다. 채팅 창에서 명령을 보내면 AI 작업 실행을 원격으로 제어할 수 있습니다.

![](../../images/previews/en/image8.png)

### Scheduled Tasks — Automated Execution

모니터링 빈도를 설정하세요: 5분마다, 매시간, 또는 매일 고정 시간에. Cron 표현식의 그래픽 스케줄러로 직관적인 자동화를 실현합니다. 정기적으로 페이지나 데이터 소스를 가져와 변경 사항을 비교하고, 주요 지표를 모니터링하며, 이상 발견 시 즉시 메시지 채널로 알림을 전송합니다.

![](../../images/previews/en/image9.png)

### Text Selection for Instant Q&A

화면의 텍스트를 선택하면 자동으로 플로팅 퀵 질문 상자에 복사됩니다. 원클릭으로 AI 어시스턴트에게 보내면 즉시 답변을 받을 수 있습니다.

![](../../images/previews/en/image10.png)

### Smart Sidebar

다른 앱 창에 스냅할 수 있는 스마트 어시스턴트입니다. 질문을 위해 서로 다른 구성의 AI 어시스턴트 간에 빠르게 전환하세요. 로봇이 연결된 지식 베이스에 따라 답변을 생성하고 원클릭으로 회신을 보낼 수 있습니다. 스마트 플로팅 팔로우로 도구 입력이 언제나 손에 있습니다.
...
질문을 반복할 필요가 없습니다. 여러 "AI 전문가"에게 동시에 질문하고 같은 인터페이스에서 나란히 답변을 비교하여 최선의 결론에 도달하세요.

![](../../images/previews/en/image13.png)

### One-Click Launcher Ball

데스크톱의 플로팅 볼을 클릭하면 ChatClaw 메인 앱 창을 즉시 실행하거나 열 수 있습니다.

![](../../images/previews/en/image14.png)

### Community & Contact Us

도움말을 받거나 개선 제안을 해주세요. WeChat: QR 코드를 스캔하여 ChatClaw 기술 교류 그룹에 참여하세요. 추가时请注明 "chatclaw".
