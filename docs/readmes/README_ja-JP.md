...
AIアシスタントに質問すれば、知識ベースをインテリジェントに検索し、関連する回答を生成します。組み込みスキルマーケットにより、AIエージェントは自律的に動作し，手動の監視は必要ありません。複雑なプロジェクト計画、ドキュメント整理、PPT生成、 멀티ステップタスク実行のいずれにおいても、AIエージェントは自律的に分析し、効率的に推進し、最終的に完全な結果を達成します。
...
それぞれ独自のペルソナ、メモリースキルを持つ複数の独立したAIエージェントを作成します。需要に応じて互いに干涉せずに切り替えます。各エージェントには「カスタマーサービス」「データアナリスト」「コピーアシスタント」などの異なる役割を割り当て、異なるスキル、知識ベース、応答スタイルで構成できます。

![](../../images/previews/en/image3.png)

### Local Knowledge Base Management

ドキュメント（TXT、PDF、Word、Excel、CSV、HTML、Markdown）をアップロードします。システムは自動的に解析、分割、ベクトル化し、プライベートナレッジベースに保存して、AIによる精密な検索と利用を可能にします。ドキュメントをフォルダとナレッジベースで整理することをサポートします。

![](../../images/previews/en/image4.png)

### Skill Manager — AI Responds at Command

5000以上のすぐに使えるAIスキルが生産性、開発ツール、マルチメディア、スマートホームなどをカバーします。プログラミングなしでAIに作業を任せます。コマンドを使用して、PCにインストール済みの機能を見つけたり、新しい拡張プラグインをインストールしたりできます。スキルマーケットでスキルを自由に閲覧してインストールできます。

![](../../images/previews/en/image5.png)

### Memory — Natural & Smarter Interactions

コンテキスト対話を有効化し、パーソナライズされたサービスを提供し、複雑なタスクを完了させ、継続的な学習と進化を実現します。ロボットは、より思慮深く、より知的なサービスを提供する成長するパートナーになります。

![](../../images/previews/en/image6.png)

### Free Model Trial

ワンクリックでChatWikiに接続し、クレジットを同期、カスタムモデルをサポート。Ollama、Google Gemini、OpenAIなどの高品质な組み込みLLM，日常業務でも專業的なシナリオでも、お好みのAIモデルを使用できます。

![](../../images/previews/en/image7.png)

### Multi-Channel Remote Control via WeChat/QQ/WhatsApp

ChatClawは複数のメッセージチャンネルをサポートし、分析結果、監視アラート、研究サマリーを直接スマホに推送します。AI処理完了後、結果は自動的に指定チャンネルに送信されます。チャットウィンドウでコマンドを送信すると、AIタスク実行をリモートコントロールできます。

![](../../images/previews/en/image8.png)

### Scheduled Tasks — Automated Execution

監視頻度を設定します：5分ごと、毎時間、または毎日的一定時刻に。cron式のグラフィカルスケジューラー。定期的にページまたはデータソースを取得し、変更を比較し、主要指標を監視し、异常発見時にメッセージチャンネルでアラートをプッシュします。

![](../../images/previews/en/image9.png)

### Text Selection for Instant Q&A

画面上のテキストを選択すると、自動的にフローティングクイック質問ボックスにコピーされます。ワンクリックでAIアシスタントに送信し、即座に回答を得られます。

![](../../images/previews/en/image10.png)

### Smart Sidebar

他のアプリウィンドウにスナップできるスマートアシスタント。異なる構成のAIアシスタントを切り替えて質問できます。ロボットは関連する知识베이스に基づいて回答を生成し、ワんクリック返信送信をサポート。スマートフローティングフォローでツール入口が常に手の届くところに。
...
同じ質問を繰り返す必要はありません。複数の「AI専門家」に同時に相談し、同じインターフェースで Repliesを並べて比較し、最適な結論に達せます。

![](../../images/previews/en/image13.png)

### One-Click Launcher Ball

デスクトップ上のフローティングボールをクリックすると、ChatClawメインアプリケーションウィンドウが即座に起動または開きます。

![](../../images/previews/en/image14.png)

### Community & Contact Us

お問い合わせやご提案を歓迎します。WeChat：QRコードをスキャンしてChatClaw技術交流グループに参加してください。追加時は「chatclaw」とご記入ください。
