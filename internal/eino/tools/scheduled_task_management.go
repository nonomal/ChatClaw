...
func (t *scheduledTaskCreatePreviewTool) Info(_ context.Context) (*schema.ToolInfo, error) {
	return &schema.ToolInfo{
		...
		ParamsOneOf: schema.NewParamsOneOfByParams(map[string]*schema.ParameterInfo{
			...
			"schedule_type":  {Type: schema.String, Desc: selectDesc("Schedule type: 'preset' (only fixed values: every_minute, every_5_minutes, every_15_minutes, every_hour, every_day_0900, every_day_1800, weekdays_0900, every_monday_0900, every_month_1_0900) or 'cron' (with cron_expr)", "调度类型: 'preset'(仅支持固定值: every_minute, every_5_minutes, every_15_minutes, every_hour, every_day_0900, every_day_1800, weekdays_0900, every_monday_0900, every_month_1_0900) 或 'cron'(需提供cron_expr)"), Required: true},
			"schedule_value": {Type: schema.String, Desc: selectDesc("Schedule value (for preset type only, e.g. 'every_day_0900'). Empty for cron type.", "调度值(仅preset类型需要，如'every_day_0900')。cron类型留空。"), Required: true},
...
func (t *scheduledTaskCreateConfirmTool) Info(_ context.Context) (*schema.ToolInfo, error) {
	return &schema.ToolInfo{
		...
		ParamsOneOf: schema.NewParamsOneOfByParams(map[string]*schema.ParameterInfo{
			...
			"schedule_type":  {Type: schema.String, Desc: selectDesc("Schedule type: 'preset' or 'cron'", "调度类型: 'preset' 或 'cron'"), Required: true},
