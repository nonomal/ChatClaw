...
(function() {
    ...
    for (const selector of inputSelectors) {
        try {
            ...
            if (input) {
                console.log('[ChatClaw] Found input with selector:', selector);
                break;
            }
        } catch (e) {}
    }
    ...
    if (!input) {
        console.warn('[ChatClaw] No input element found');
        return;
    }
    ...
    const fillInput = () => {
        if (input.tagName === 'TEXTAREA' || input.tagName === 'INPUT') {
            ...
            if (isQwen) {
                console.log('[ChatClaw] Using Slate.js-compatible input method for Qwen...');
                ...
                if (!container) {
                    console.log('[ChatClaw] Qwen container not found, using fallback method...');
                }
                ...
                setTimeout(() => {
                    ...
                    if (!pasteResult) {
                        console.log('[ChatClaw] Paste blocked, trying execCommand...');
                    ...
                    console.log('[ChatClaw] Qwen message injected successfully');
                }, 50);
        ...
        input.focus();
        console.log('[ChatClaw] Message injected successfully');
    };
    ...
    setTimeout(() => {
            ...
            try {
                ...
                for (const btn of btns) {
                    if (!btn.disabled && btn.offsetParent !== null) {
                        sendButton = btn;
                        console.log('[ChatClaw] Found send button with selector:', selector);
                        break;
                    }
                }
                if (sendButton) break;
            ...
            for (let i = 0; i < 5 && parent; i++) {
                ...
                for (const btn of buttons) {
                    ...
                    if (isVisible && isNotDisabled && looksLikeSend) {
                        sendButton = btn;
                        console.log('[ChatClaw] Found send button near input');
                        break;
                    }
                }
                if (sendButton) break;
        ...
        const sendMessage = () => {
            ...
            if (isQwen) {
                console.log('[ChatClaw] Using button click to send (Qwen Slate.js)...');
                ...
                if (qwenSendBtn && !qwenSendBtn.classList.contains('disabled')) {
                    console.log('[ChatClaw] Clicking Qwen send button...');
                    qwenSendBtn.click();
                } else if (sendButton) {
                    console.log('[ChatClaw] Qwen operateBtn not found or disabled, trying fallback button...');
                    sendButton.click();
                } else {
                    // 最后尝试 Enter 键
                    console.log('[ChatClaw] No send button found, trying Enter key...');
                    input.focus();
            ...
            if (isDoubao || isDeepSeek || isKimi || isYuanbao || isGLM) {
                ...
                setTimeout(() => {
                    if (sendButton) {
                        console.log('[ChatClaw] Enter might not work, trying button click...');
                        sendButton.click();
                    }
                }, 200);
            ...
            // 其他网站优先使用按钮
            if (sendButton) {
                console.log('[ChatClaw] Clicking send button...');
                sendButton.click();
            } else {
