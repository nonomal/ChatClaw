...
本项目使用 Cursor Hook 自动归档机制，确保 AGENTS.md 持续迭代。

#### 触发方式

1. **自动触发**：AI 完成任务后（检测到文件变更或多次工具调用），会自动询问"是否归档"
2. **手动触发**：在任意对话中输入"归档"即可立即触发
3. **超时行为**：自动询问默认 30 秒后自动跳过，不阻塞工作流
...
2. **触发迭代**：向 AI 发送迭代提示，要求检查以下内容并更新 AGENTS.md：
...
2. AI 会先执行归档，然后检查所有规则文件、代码结构，输出需要更新的内容
3. AI 会根据检查结果更新 AGENTS.md
